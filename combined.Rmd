...
stats = subset(benches,select = -c(X1,GAME_ID,TEAM_CITY,TEAM_ID,PLAYER_ID,PLAYER_NAME,Home_team,COMMENT))
stats = na.omit(stats)
...
team_pos = function(df){
  model = glm(WIN~MIN + FGM + FGA + FG_PCT + FG3M + FG3A + FG3_PCT + FTM + FTA + FT_PCT + OREB + DREB + REB + AST + STL + BLK + TO + PF + PTS + PLUS_MINUS, data=df, family = binomial(logit))
  return(model)
}
pospred = stats.train %>%
...
rf.team_pos = function(df){
  model = randomForest(as.numeric(WIN)~MIN + FGM + FGA + FG_PCT + FG3M + FG3A + FG3_PCT + FTM + FTA + FT_PCT + OREB + DREB + REB + AST + STL + BLK + TO + PF + PTS + PLUS_MINUS, data=df, ntree=25)
  return(model)
}
...
svm.team_pos = function(df){
  model = svm(WIN~MIN +FGM + FGA + FG_PCT + FG3M + FG3A + FG3_PCT + FTM + FTA + FT_PCT + OREB + DREB + REB + AST + STL + BLK + TO + PF + PTS + PLUS_MINUS, data=df, kernel='radial')
  return(model)
}
...
set.seed(1)
rf.pos = function(df){
  model = randomForest(as.numeric(WIN)~MIN +FGM + FGA + FG_PCT + FG3M + FG3A + FG3_PCT + FTM + FTA + FT_PCT + OREB + DREB + REB + AST + STL + BLK + TO + PF + PTS + PLUS_MINUS, data=df, mtry=5, ntree=25)
  return(model)
}
rf.pred = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(rf.pos(.), newdata = test_benches))
rfpred.train = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(rf.pos(.), newdata = train_benches))
...
svm.pred = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(svm.pos(.), newdata = test_benches))
svmpred.train = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(svm.pos(.), newdata = trains_benches))
...
```{r}
benches = read_csv('2019-2020_ANNO_games_details_BENCH.csv')
d_length = dim(benches)[1]
benches$WIN = as.numeric(benches$WIN)
benches$Home_team = as.numeric(benches$Home_team)
train_index = sample(d_length, d_length * 0.7)
train_benches = benches[train_index,]
test_benches = benches[-train_index,]
```
...
#
my_rf <- function(df) {
    fit = randomForest(WIN ~ MIN + FGM + FGA + FG_PCT + FG3M + FG3A + FG3_PCT + FTM + FTA + FT_PCT + OREB + DREB + REB + AST + STL + BLK + TO + PF + PTS + PLUS_MINUS, data = df, mtry = 5, importance=TRUE, ntrees=25)
    return(fit)
}

rf_preds = train_benches %>% 
  group_by(START_POSITION) %>%
  do(pred = predict(my_rf(.), newdata = test_benches))

rf_preds_train = train_benches %>% 
  group_by(START_POSITION) %>%
  do(pred = predict(my_rf(.), newdata = train_benches))

#
rf.error_rate_test = rep(0, 5)
for (x in 1:5) {
  rf_pred_list = rf_preds$pred[[x]]
  rf_pred = rep(0, length(rf_pred_list))
  threshold <- 0.5
  rf_pred[rf_pred_list > threshold] = 1
  rf_error_rate_test[x] = mean(rf_pred != test_benches$WIN)
}
...
for (x in 1:5) {
  ...
  threshold <- 0.5
  rf_pred[rf_pred_list > threshold] = 1
  rf_error_rate_train[x] = mean(rf_pred != train_benches$WIN)
}
