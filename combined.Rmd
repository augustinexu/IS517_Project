...
pospred = train_benches %>%
  group_by(TEAM_ABBREVIATION,START_POSITION)%>%
  do(pred = predict(team_pos(.), newdata = test_benches))
pospred.train = train_benches %>%
  group_by(TEAM_ABBREVIATION,START_POSITION)%>%
  do(pred = predict(team_pos(.), newdata = train_benches))
...
for (x in 1:148){
  ...
  error_rate[x] = mean(pred != test_benches$WIN)
}
...
for (x in 1:148){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  error_rate_train[x] = mean(pred != train_benches$WIN)
}
```
...
rf.pospred = train_benches %>%
  group_by(TEAM_ABBREVIATION,START_POSITION)%>%
  do(pred = predict(rf.team_pos(.), newdata = test_benches))
rfpospred.train = train_benches %>%
  group_by(TEAM_ABBREVIATION,START_POSITION)%>%
  do(pred = predict(rf.team_pos(.), newdata = train_benches))
...
for (x in 1:148){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  rfpos.error_rate[x] = mean(pred != test_benches$WIN)
}
...
for (x in 1:148){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  rfpos.error_rate_train[x] = mean(pred != train_benches$WIN)
}
```
...
stats.train = stats[train_index,]
stats.test = stats[-train_index,]
set.seed(1)
...
svmpospred.train = train_benches %>%
  group_by(TEAM_ABBREVIATION,START_POSITION)%>%
...
for (x in 1:148){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  svmpos.error_rate[x] = mean(pred != test_benches$WIN)
}
...
for (x in 1:148){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  svmpos.error_rate_train[x] = mean(pred != train_benches$WIN)
}
```
...
teampred = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(team(.), newdata = test_benches))
teampred.train = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(team(.), newdata = train_benches))
...
for (x in 1:5){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  error_rate_test[x] = mean(pred != test_benches$WIN)
}
...
for (x in 1:5){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  rf.error_rate[x] = mean(pred != test_benches$WIN)
}
...
for (x in 1:5){
  ...
  threshold = 0.5
  pred[pred_list > threshold] = 1
  rf.error_rate_train[x] = mean(pred != train_benches$WIN)
}
```
...
svm.pred = stats.train %>%
  group_by(START_POSITION)%>%
  do(pred = predict(svm.pos(.), newdata = stats.test))
svmpred.train = train_benches %>%
  group_by(START_POSITION)%>%
  do(pred = predict(svm.pos(.), newdata = stats.train))
